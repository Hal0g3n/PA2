...
public class PeerService {
  ...
  public static final int NUMERIC_ID_LEN = 5; // no more than 32 peers in the demo
  ...
  public void stopService() {
    ...
    for (int i = 0; i < NUMERIC_ID_LEN; i++) {
      ...
      if (clockwise != null && anticlockwise != null) {
        ...
        becomeNeighbour =
          new Message(clockwise, String.format("clockwise:%d", i), "neighbour");
  ...
  private void routeByNumericID(RouteMessage msg) {
    ...
    if (head.getNumericID() == msg.getDestId() || msg.isFinalDestination()) {
      System.out.println(
        "Looks like I'm the intended peer for this message. On to processing!"
      );
    ...
    if (msg.getStartNode() != null) {
      System.out.printf(
        "Source node is no longer null, %s == %s?\n",
        Encoding.bitSetToString(head.getNumericID(), NUMERIC_ID_LEN),
        Encoding.bitSetToString(
          msg.getStartNode().getNumericID(),
          NUMERIC_ID_LEN
        )
      );
      if (head.getNumericID().equals(msg.getStartNode().getNumericID())) {
        ...
        System.out.println(
          "We went in a loop, dammit! Sending back to the best node"
        );
    ...
    System.out.printf(
      "The common prefix length between %s and %s is %d\n",
    ...
    if (
      nextPeer == null
    ) { // well, we're working with an empty ring
    ...
    System.out.printf(
      "Forwarding along the ring to Peer %d @ %s, see ya\n",
      nextPeer.getPeerNum(),
      nextPeer.getAddress()
    );
  ...
  private void handleMessage(Message msg) {
    ...
    switch (msgContent) {
      case "insertion" -> {
        ...
        System.out.printf(
          "The joining node has numeric id %d\n",
  ...
  private void insertNode(JoinMessage joinMessage) {
    System.out.printf(
      "Received join message at ring lvl %d\n",
    ...
    while (joinMessage.getRingLvl() >= 0) {
      // keep going lower
      Connection clockwise = head.getClockwiseNeighbour(
        joinMessage.getRingLvl()
      );
      if (clockwise != null) {
        System.out.printf(
          "We are trying to do an insert of %d [%d] %d\n",
          head.getPeerNumber(),
      ...
      if (
        clockwise == null || // there should be a short circuit
        ...
        if (clockwise != null) joinMessage.setRingClockwise(
          ringLvl,
          clockwise
        ); else joinMessage.setRingClockwise(ringLvl, selfConnection);
        System.out.printf(
          "Set c to %d and ac to %d\n",
  ...
  private void insertIntoRings(JoinMessage joinMessage) {
    for (int i = 0; i <= joinMessage.getOriginalLvl(); i++) { // numID len is max ring depth
      System.out.printf(
        "Making request at lvl %d where c is %d and ac is %d\n",
        i,
      ...
      if (joinMessage.getRingAnticlockwise(i) != null) {
        System.out.printf(
          "Requesting insertion from %d\n",
          Encoding.BitSetToInt(
            joinMessage.getRingAnticlockwise(i).getNumericID()
          )
        );
        ...
        connectionService.sendMessage(
          becomeNeighbour,
          joinMessage.getRingAnticlockwise(i)
        );
      }
      ...
      if (joinMessage.getRingClockwise(i) != null) {
        ...
        connectionService.sendMessage(
          becomeNeighbour,
          joinMessage.getRingClockwise(i)
        );
      }
    }
  }
  ...
  public void handleImmediateMessage(ChannelHandlerContext ctx, Message msg) {
    ...
    switch (msgCommand) {
      case "register" -> {
        ...
        peerNumberCounter++;
        int numID = ThreadLocalRandom
          .current()
          .nextInt(0, (1 << NUMERIC_ID_LEN));
        while (usedId.containsKey(numID)) numID =
          ThreadLocalRandom.current().nextInt(0, (1 << NUMERIC_ID_LEN));
        usedId.put(numID, true);
        System.out.printf(
          "Received a registration, assigning IDs %d %d\n",
          peerNumberCounter,
          numID
        );
      ...
      case "assignedNum" -> {
        ...
        this.selfConnection.setNumericID(
            Encoding.intToBitSet(Integer.parseInt(tokens[1]), NUMERIC_ID_LEN)
          );
        this.head.setNumericID(
            Encoding.intToBitSet(Integer.parseInt(tokens[1]), NUMERIC_ID_LEN)
          );
        ...
        System.out.printf(
          "Peer registration complete with assigned IDs %s %s\n",
          tokens[0],
          tokens[1]
        );
      }
      ...
      case "getPeerID" -> {
        ...
        Message responseMsg = new Message(
          ...
          String.format(
            "%d %d",
            peerNumberCounter,
            Encoding.BitSetToInt(head.getPeerID())
          ),
          "retPeerID"
        );
        ctx.writeAndFlush(responseMsg);
