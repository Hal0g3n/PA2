...
//    }
    ...
    public void sendMsg(int targetId, String message){
        ...
        sendMsg(head.id, targetId, message, "127.0.0.1", 8080);
    }

    private void sendMsg(int sourceId, int targetId, String message, String ip, int port){
        connectionService.sendMessage(String.format("ping %d %d %d %s", sourceId, head.id, targetId, message), ip, port);
    }
    ...
    public void handleMessage(ChannelHandlerContext ctx, String msg){
        ...
        switch (command) {
            ...
            case "ping" -> { // ping sourceID fromID targetID msg
                ...
                if(head.id == targetID){
                    ...
                    String demoResponse = String.format(
                            "pong %d %d %d %s",
                            targetID, head.id, sourceID, pingMsg);
                    ByteBuf out = ctx.alloc().buffer(demoResponse.length() * 2);
                    out.writeBytes(Encoding.str_to_bb(demoResponse));
                    ctx.writeAndFlush(out);
                    ctx.close();
            ...
            case "pong" -> { // pong sourceID fromID targetID msg
                ...
                if(head.id == targetID){ // i am the one who pong'd
                    ...
                    connectionService.sendMessage(String.format(
                            "pong %d %d %d %s",
                            sourceID, head.id, targetID, pongMsg),
