...
public class AVL_YX<T extends Comparable<? super T>> {
  ...
  private T getMin(Node<T> node) {
    return node.neighbours[0] == null
      ? node.getItem()
      : getMin(node.neighbours[0]);
  }
  ...
  private Node<T> balanceSubTree(Node<T> root) {
    ...
    if (getBalance(root) > 1) {
      ...
      if (getBalance(root.neighbours[0]) < 0) root.neighbours[0] =
        leftRotate(root.neighbours[0]);
    ...
    // Right subtree to tall
      ...
      if (getBalance(root.neighbours[0]) > 0) root.neighbours[1] =
        rightRotate(root.neighbours[1]);
  ...
  public void insert(T item) {
    root = insert(new Node(item, 2), root);
  }
  ...
  private Node<T> insert(Node<T> item, Node<T> root) {
    ...
    if (item.getItem().compareTo(root.getItem()) < 0) root.neighbours[0] =
      insert(item, root.neighbours[0]);
    if (item.getItem().compareTo(root.getItem()) > 0) root.neighbours[1] =
      insert(item, root.neighbours[1]);
  ...
  private Node<T> delete(T item, Node<T> root) {
    ...
    if (root.getItem().compareTo(item) > 0) root.neighbours[0] =
      delete(item, root.neighbours[0]);
    if (root.getItem().compareTo(item) < 0) root.neighbours[1] =
      delete(item, root.neighbours[1]);
