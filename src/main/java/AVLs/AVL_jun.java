...
public class AVL_jun<T extends Comparable<? super T>> {
  ...
  private Node<T> insert(T item, Node<T> curr) {
    ...
    int neighbourSide = ((Comparable) item).compareTo(curr.getItem()) > 0
      ? 1
      : 0; // get left or right index
    curr.neighbours[neighbourSide] =
      insert(item, curr.neighbours[neighbourSide]); // insert on whatever side
    ...
    if (curr.getItem().compareTo(item) == 0) { // we found the node
      ...
      switch (childSwitch) {
        ...
        case 3 -> {
          ...
          while (inOrderSucc.neighbours[0] != null) inOrderSucc =
            inOrderSucc.neighbours[0]; // keep going keep going
      ...
      int neighbourSide = ((Comparable) item).compareTo(curr.getItem()) > 0
        ...
        : 0; // get left or right index
      curr.neighbours[neighbourSide] =
        delete(item, curr.neighbours[neighbourSide]); // insert on whatever side
    }
    return balanceTree((Node<T>) curr);
  }
  ...
  private Node<T> balanceTree(Node<T> curr) {
    ...
    balanceSwitch =
      Integer.signum(
        balanceSwitch >= -1 && balanceSwitch <= 1 ? 0 : balanceSwitch
      ); // uhuh
    ...
    if (balanceSwitch > 0) { // base is left
      if (getBalance(curr.neighbours[0]) < 0) curr.neighbours[0] =
        leftRotate(curr.neighbours[0]); // turn it into a left-left
      return rightRotate(curr);
    } else {
      if (getBalance(curr.neighbours[0]) > 0) curr.neighbours[1] =
        rightRotate(curr.neighbours[1]); // turn it into a right-right
