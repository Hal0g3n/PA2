...
public class AVL_aloysus<T extends Comparable<? super T>> {
  ...
  private Node<T> fixTree(T item, Node<T> root) {
    ...
    // left left
    if (
      balance > 1 && item.compareTo(root.neighbours[0].getItem()) < 0
    ) return rightRotate(root);
    // right right
    if (
      balance < -1 && item.compareTo(root.neighbours[1].getItem()) > 0
  ...
  private Node<T> delete(T item, Node<T> curr) {
    ...
    if (item.compareTo(curr.getItem()) < 0) curr.neighbours[0] =
      delete(item, left); else if (
      item.compareTo(curr.getItem()) > 0
    ) curr.neighbours[1] = delete(item, right); else {
      // one or no children
