...
public class RTreeNode<T extends RTreeEntry> extends model.Node<List<T>> {
   ...
   */
  public static boolean isOverlap(Range<Double>[] r1, Range<Double>[] r2) {
    if (r1.length != r2.length) throw new IllegalArgumentException(
      "输入的顶点的范围数不一样"
    );

    for (int i = 0; i < r1.length; i++) {
      if (
        r1[i].getMin() > r2[i].getMax() || r2[i].getMin() > r1[i].getMax()
      ) return false;
    }
   ...
   */
  public static boolean isInRange(Range<Double>[] r1, Double[] coords) {
    if (r1.length != coords.length) throw new IllegalArgumentException(
      "输入的范围数不一样"
    );

    for (int i = 0; i < r1.length; i++) {
      if (
        coords[i] < r1[i].getMin() || r1[i].getMax() < coords[i]
      ) return false;
    }
   ...
   */
  public static double getArea(RTreeNode node) {
    double area = 1.0f;
    for (Range<Double> range : node.ranges) area *=
  ...
  public void tighten() {
    ...
    if (this.leaf) {
      ...
      List<Double[]> elements = item
        .stream()
        .map(RTreeEntry::getParamValues)
      ...
      for (int i = 0; i < ranges.length; i++) { // For each dimension
        ...
        if (numChildren > 1) {
          ...
          RTreeNode<T> child = (RTreeNode<T>) (
            neighbours[0] == null ? neighbours[1] : neighbours[0]
          );
  ...
  double getAreaExpansion(T e) {
    ...
    if (e_vals.length != ranges.length) throw new IllegalArgumentException(
      "e的参数数不对"
    );
    ...
    for (int i = 0; i < e_vals.length; i++) {
      // Expand end point
      if (this.ranges[i].getMax() < e_vals[i]) expanded *=
        e_vals[i] - this.getRanges()[i].getMin();

      // Expand start point
      if (this.ranges[i].getMin() > e_vals[i]) expanded *=
        this.ranges[i].getMax() - e_vals[i];
    }
  ...
  double getAreaExpansion(RTreeNode<T> e) {
    ...
    if (e.ranges.length != ranges.length) throw new IllegalArgumentException(
      "e的参数数不对"
    );
    ...
    for (int i = 0; i < e.ranges.length; i++) {
      ...
      // Expand end point
      if (this.ranges[i].getMax() < e.ranges[i].getMax()) delta +=
        e.ranges[i].getMax() - this.getRanges()[i].getMax();

      // Expand start point
      if (this.ranges[i].getMin() > e.ranges[i].getMin()) delta +=
        this.ranges[i].getMin() - e.ranges[i].getMin();
