...
public class RTree<T extends RTreeEntry> {
  ...
  public RTree(int maxEntries, int minEntries, int numDims) {
    if (minEntries * 2 > maxEntries) throw new IllegalArgumentException(
      "minEntries太大"
    );
  ...
  private RTreeNode<T> buildRoot(boolean asLeaf) {
    ...
    // ± sqrt(MAX_VALUE)
    for (int i = 0; i < this.numDims; i++) ranges[i] =
      new Range<>(
  ...
  public List<T> search(Range<Double>[] ranges) {
    if (ranges.length != numDims) throw new IllegalArgumentException(
      "输入的数组大小不对"
    );
  ...
  private void search(
    ...
    if (
      n.isLeaf()
    ) for (T e : n.getItem()) { // n is leaf, contains entries // For each entry
      if (RTreeNode.isInRange(ranges, e.getParamValues())) results.add(e);
    } else for (int i = 0; i < maxChildren; ++i) { // If not leaf, travel down the children // 2 children
      // Subtree does not contain the query range or child does not exist
      if (
        n.neighbours[i] == null ||
        !RTreeNode.isOverlap(
          ranges,
          ((RTreeNode<T>) n.neighbours[i]).getRanges()
        )
      ) continue;
  ...
  public boolean delete(GhostNode<T> node) {
    if (node.getRanges().length != numDims) throw new IllegalArgumentException(
      "输入的范围大小不对"
    );
  ...
  private RTreeNode<T> findLeaf(RTreeNode<T> n, Double[] params, T entry) {
    if (
      params.length != n.getRanges().length
    ) throw new IllegalArgumentException("输入的数组大小不对");

    if (n.isLeaf()) for (T e : n.getItem()) {
      if (!e.equals(entry)) continue;
      return n; // RTreeNode found
    } else for (int i = 0; i < maxChildren; ++i) {
      // ignore null children
      if (n.neighbours[i] == null) continue;
      // If child does not include entry range
      System.out.println(Arrays.toString(params));
      System.out.println(
        Arrays.toString(((RTreeNode<T>) n.neighbours[i]).getRanges())
      );
      if (
        !RTreeNode.isInRange(
          ((RTreeNode<T>) n.neighbours[i]).getRanges(),
      ...
      RTreeNode<T> result = findLeaf(
        (RTreeNode<T>) n.neighbours[i],
        params,
        entry
      );
  ...
  private void condenseTree(RTreeNode<T> n) {
    ...
    while (n != root) {
      if (n.isLeaf() && (n.getItem().size() < minEntries)) {
        ...
        ((RTreeNode<T>) n.neighbours[3]).removeChild(
            n.getId().get(n.getId().size() - 1) ? 1 : 0
          );
    ...
    if (n.neighbours[0] == null || n.neighbours[1] == null) {
      ...
      RTreeNode<T> child = (RTreeNode<T>) (
        n.neighbours[0] == null ? n.neighbours[1] : n.neighbours[0]
      );
  ...
  public void insert(T entry) {
    if (
      entry.getParamValues().length != numDims
    ) throw new IllegalArgumentException("输入的大小不对");
  ...
  public void insert(GhostNode<T> n_node) {
    ...
    for (int lvl = 0; lvl < n_node.getId().size(); ++lvl) {
      ...
      if (
        parent.neighbours[n_node.getId().get(lvl) ? 1 : 0] == null ||
        !(
          parent.neighbours[n_node.getId().get(lvl)
              ? 1
              : 0] instanceof GhostNode
        )
      ) {
        // Add to parent
        ...
        (RTreeNode<T>) root.neighbours[n_node.getId().get(lvl) ? 1 : 0];
    }
  ...
  private RTreeNode<T>[] splitNode(RTreeNode<T> n) {
    ...
    for (int i = 0; i < numDims; ++i) {
      newRanges[i] =
        new Range<>(n.getRanges()[i].getMin(), n.getRanges()[i].getMax());
    }
    RTreeNode<T>[] n_nodes = new RTreeNode[] {
      n,
      new RTreeNode<>(
        new LinkedList<>(),
        newRanges,
        false,
        (RTreeNode<T>) n.neighbours[3]
      ),
    };

    // Add children to parent
    if (n_nodes[1].neighbours[3] != null) (
      (RTreeNode<T>) n_nodes[1].neighbours[3]
    ).addChild(n_nodes[1]);
    ...
    n.neighbours =
      new RTreeNode[] { null, null, null, (RTreeNode<T>) n.neighbours[3] }; // Clear the neighbours
    ...
    while (!cc.isEmpty()) {
      ...
      if (
        n_nodes[0].getItem().size() < n_nodes[1].getItem().size()
      ) n_nodes[0].addChild(c); else n_nodes[1].addChild(c);
    }
   ...
   */
  private ArrayList<RTreeNode<T>> pickNodeSeeds(
    LinkedList<RTreeNode<T>> children
  ) {
    ...
    bestPair.add(null);
    bestPair.add(null); // this is an array but java dont like a real array
    ...
    for (int dim = 0; dim < numDims; dim++) { // For each dimension
      ...
      double dimUb = -1.0f * Double.MAX_VALUE, dimMaxLb =
        -1.0f * Double.MAX_VALUE;
      ...
      for (RTreeNode<T> e : children) {
        ...
        if (e.getRanges()[dim].getMin() < dimLb) dimLb =
          e.getRanges()[dim].getMin(); // The minimum
        if (e.getRanges()[dim].getMax() > dimUb) dimUb =
          e.getRanges()[dim].getMax(); // The maximum
  ...
  private RTreeNode<T>[] splitLeaf(RTreeNode<T> n) {
    ...
    while (!cc.isEmpty()) {
      if (
        (n_nodes[0].getItem().size() >= minEntries) &&
        ...
        (n_nodes[1].getItem().size() >= minEntries) &&
      ...
      if (
        n_nodes[0].getItem().size() < n_nodes[1].getItem().size()
      ) n_nodes[0].addEntry(c); else n_nodes[1].addEntry(c);
    }
  ...
  private ArrayList<T> pickLeafSeeds(LinkedList<T> entries) {
    ...
    Double[][] elements = Arrays.copyOf(
      entries.stream().map(RTreeEntry::getParamValues).toArray(),
      entries.size(),
      Double[][].class
    );
    ...
    bestPair.add(null);
    bestPair.add(null); // this is so inelegant but java doesn't like generic arrays
  ...
  public void clear() {
    root = buildRoot(true);
  } // Garbage Collector will clear the rest
}
