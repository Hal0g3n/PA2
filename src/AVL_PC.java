    ...
    private Node<T> delete(T item, Node<T> curr){
        ...
        if (curr.getItem() == item) {
            ...
            if (parent == curr) {
                ...
                parent.neighbours[0] = null;

            // balance check: here it is always either left-right or left-left
            if (getBalance(curr) > 1) {
                if (getBalance(curr.neighbours[1]) < 0)
                    curr.neighbours[1] = leftRotate(curr.neighbours[1]);
            ...
            if (getBalance(curr) > 1) {
                ...
                if (getBalance(curr.neighbours[0]) > 0)
                    curr.neighbours[0] = rightRotate(curr.neighbours[0]);
                curr = leftRotate(curr);
            }
        }
        return curr;
